...
function setupAuthListeners() {
    ...
    if (authForm) {
        authForm.onsubmit = handleMagicAuth;
    }
...
async function handleMagicAuth(e) {
    ...
    try {
        ...
        if (loginError) {
            let msg = loginError.message || String(loginError);

            // Se credenciais inválidas, pode ser senha errada OU usuário não existe.
            if (msg.includes('Invalid login credentials')) {
                // Tenta Cadastrar
                const { data, error: signupError } = await supabase.auth.signUp({
                    email,
                    password,
                    options: { data: { full_name: 'Ateliê' } }
                });

                if (signupError) {
                    let sigMsg = signupError.message || String(signupError);
                    if (sigMsg.includes('User already registered')) {
                        // Usuário EXISTE, mas a senha inicial no signIn estava errada!
                ...
                // Cadastro deu certo!
                if (!data.session) {
                    alert("Bem-vinda! Conta criada com sucesso.");
                }
            } else {
                throw loginError;
            }
...
async function handleForgotPassword(e) {
    ...
    if (!email) {
        alert("Por favor, preencha o seu E-mail no campo acima antes de clicar em Esqueci a Senha.");
        return;
    }
    ...
    try {
        ...
        alert("Enviamos um e-mail com instruções para você redefinir sua senha. Verifique sua caixa de entrada e o lixo eletrônico (spam).");
    } catch (err) {
        alert("Erro ao enviar recuperação: " + (err.message || String(err)));
    } finally {
        setLoading(false);
    }
}
