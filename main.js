/**
 * ENCANTOS DA NÊ - GESTÃO PRO CLOUD v8.0 (Magic Auth)
...
async function initApp() {
    try {
        if (!window.supabase) {
            alert("⚠️ Supabase não carregou. Verifique a internet.");
            return;
        }
        ...
        alert("Erro fatal ao iniciar: " + e.message);
    }
}
...
function setAuthMode(mode) {
    ...
    const toggleP = document.getElementById('toggle-auth-p');
    const forgotP = document.getElementById('forgot-password-p');
    const backP = document.getElementById('back-login-p');

    if (mode === 'login') {
        title.innerText = 'Bem-vinda de volta!';
        subtitle.innerText = 'Acesse sua conta para sincronizar seu ateliê.';
        btn.innerText = 'Acessar';
        nameGroup.classList.add('hidden');
        passGroup.classList.remove('hidden');
        passInput.required = true;
        toggleP.classList.remove('hidden');
        toggleP.innerHTML = 'Não tem conta? <a href="#" id="toggle-auth-link" style="color: var(--primary);">Cadastrar agora</a>';
        forgotP.classList.remove('hidden');
        backP.classList.add('hidden');
    } else if (mode === 'register') {
        title.innerText = 'Crie sua conta';
        subtitle.innerText = 'Comece a gerenciar seu ateliê na nuvem.';
        btn.innerText = 'Cadastrar';
        nameGroup.classList.remove('hidden');
        passGroup.classList.remove('hidden');
        passInput.required = true;
        toggleP.classList.remove('hidden');
        toggleP.innerHTML = 'Já tem conta? <a href="#" id="toggle-auth-link" style="color: var(--primary);">Acessar</a>';
        forgotP.classList.add('hidden');
        backP.classList.add('hidden');
    } else if (mode === 'forgot') {
        title.innerText = 'Recuperar Senha';
        subtitle.innerText = 'Insira seu e-mail para receber as instruções.';
        btn.innerText = 'Enviar Instruções';
        nameGroup.classList.add('hidden');
        passGroup.classList.add('hidden');
        passInput.required = false;
        toggleP.classList.add('hidden');
        forgotP.classList.add('hidden');
        backP.classList.remove('hidden');
    }
    
    // Reattach listener since innerHTML destroys it
...
async function handleAuthSubmit(e) {
    ...
    const name = document.getElementById('auth-name') ? document.getElementById('auth-name').value || 'Ateliê' : 'Ateliê';
        ...
        if (authMode === 'login') {
            ...
            if (error) {
                if (error.message.includes('User already registered')) {
                     throw new Error("Este e-mail já possui cadastro. Use a tela de login.");
                }
                throw error;
            }
            if (!data.session) {
                alert("Cadastro realizado! Por favor, verifique seu e-mail (e a caixa de spam) para confirmar a conta antes de fazer o login.");
                setAuthMode('login');
            }
        }
            ...
            alert("Enviamos um e-mail com instruções! Verifique sua caixa de entrada e spam.");
            setAuthMode('login');
        }
    } catch (err) {
        let finalMsg = err.message || String(err);
        if (finalMsg.includes('Password should be at least')) finalMsg = "A senha deve ter pelo menos 6 caracteres.";
        alert("Ops: " + finalMsg);
    } finally {
        setLoading(false);
    }
