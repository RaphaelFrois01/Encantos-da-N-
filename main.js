...
async function handleAuthSubmit(e) {
    e.preventDefault();
    console.log("[Auth] Iniciando submissão do formulário...");

    const email = document.getElementById('auth-email').value;
    const password = document.getElementById('auth-password').value;
    const nameInput = document.getElementById('auth-name');
    const name = nameInput ? nameInput.value || 'Ateliê' : 'Ateliê';
    ...
    try {
        if (authMode === 'login') {
            ...
            console.log("[Auth] Tentando Recuperar Senha...");
        ...
        console.error("[Auth] Erro detectado:", err);
        let msg = err.message;
