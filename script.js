...
window.onerror = function (msg) {
    if (!msg.includes('lucide')) {
        alert("Erro no aplicativo: " + msg);
    }
};
...
document.addEventListener("DOMContentLoaded", async () => {
    try {
        if (!window.supabase) {
            alert("⚠️ Erro: Conexão com Supabase não carregou. Verifique sua internet ou limpe o cache.");
            return;
        }
...
async function handleAuthSubmit(e) {
    ...
    try {
        if (state.authMode === 'login') {
            ...
            // Força feedback visual de sucesso
            alert("🎉 Cadastro realizado com sucesso!");

            // Se o Supabase não logar na mesma hora, ele avisa
            if (!data.session) {
                alert("O sistema requer que você faça Login com a conta que acabou de criar.");
        ...
        let msg = err.message;
        if (msg.includes('Password should be at least 6 characters')) msg = "A senha deve ter pelo menos 6 caracteres.";
...
function renderAuthUI() {
    ...
    if (isLogin) nameGroup.classList.add('hidden');
    else nameGroup.classList.remove('hidden');
}
